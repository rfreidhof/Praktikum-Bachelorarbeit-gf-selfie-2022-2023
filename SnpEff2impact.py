...
#This program is designed to work with files created by SnpEff as of 7.10.2022. The SnpSift files have the extracted fields: CHROM ANN[*].GENEID POS ANN[*].IMPACT ANN[*].EFFECT ANN[*].BIOTYPE 

zerteiler = ArgumentParser()

zerteiler.add_argument("--in1", dest="in1", required=True, help=".ann.vcf.tsv file created bz SnpSift. Assumed 50 percent fertility.")
zerteiler.add_argument("--in2", dest="in2", required=True, help=".ann.vcf file created bz SnpSift. Assumed 100 percent fertility.")
...
#list(set(my_list)) leaves only unique elements.
for (gene,start,stop) in top_genes:
    ...
    if len(high) > 0:
        percentile, effect = zip(*high)
        hundred = "HIGH\t" + high[0][0] + "\t" + high[0][1]
    elif len(moderate) > 0:
        percentile, effect = zip(*moderate)
